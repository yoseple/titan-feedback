...
const MealCard = ({ meal, isSelected, onToggle, onChefMode, onEdit, onDelete }) => {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden mb-3 transition-all duration-200 ${isSelected ? 'bg-red-900/10 border-red-500/50' : 'bg-gray-800 border-gray-700'}`}>
      <div className="p-4 flex justify-between items-center cursor-pointer active:bg-gray-700/50" onClick={() => setIsExpanded(!isExpanded)}>
        ...
        <div className="flex items-center gap-3 shrink-0">
           <button onClick={(e) => { e.stopPropagation(); onToggle(); }} className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all ${isSelected ? 'bg-red-600 border-red-600 text-white' : 'border-gray-500 text-gray-500'}`}>{isSelected ? <Check className="w-4 h-4"/> : <Plus className="w-4 h-4"/>}</button>
        ...
        <div className="p-4 bg-gray-900/30 border-t border-gray-700/50 animate-in slide-in-from-top-2">
           <div className="flex justify-between items-center mb-3">
              ...
              <div className="flex gap-2">
                <button onClick={(e) => { e.stopPropagation(); onEdit(meal); }} className="p-1.5 text-gray-400 hover:text-white bg-gray-800 rounded"><Edit2 className="w-3 h-3"/></button>
                <button onClick={(e) => { e.stopPropagation(); onDelete(meal.id); }} className="p-1.5 text-gray-400 hover:text-red-500 bg-gray-800 rounded"><Trash2 className="w-3 h-3"/></button>
