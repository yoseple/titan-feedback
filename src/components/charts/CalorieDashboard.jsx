...
const MacroBar = ({ label, current, goal, colorClass, icon: Icon }) => {
  ...
  return (
    <div className="flex-1 bg-gray-900/50 rounded-lg p-2 border border-gray-700/50">
      ...
      <div className="flex items-end justify-between mb-1"><span className={`text-sm font-bold leading-none ${over ? 'text-red-400' : 'text-white'}`}>{current}g</span><span className="text-[9px] text-gray-500">/ {goal}g</span></div>
      <div className="h-2 w-full bg-gray-700 rounded-full overflow-hidden"><div className={`h-full ${over ? 'bg-red-500' : colorClass.replace('text-', 'bg-')}`} style={{ width: `${progress}%` }}></div></div>
    </div>
  );
};
...
const CalorieDashboard = ({ consumed, goal, protein, proteinGoal, carbs, carbsGoal, fats, fatsGoal }) => {
  ...
  return (
    <div className="bg-gray-800 p-5 rounded-2xl border border-gray-700 mb-6 shadow-lg relative overflow-hidden">
      {/* Ambient glow reflects status (emerald under target, red over) instead of always red. */}
      <div className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-3xl -mr-10 -mt-10 ${remaining < 0 ? 'bg-red-600/10' : 'bg-emerald-600/10'}`}></div>
      ...
      <div className="grid grid-cols-3 gap-3 relative z-10">
        <MacroBar label="Protein" current={protein} goal={proteinGoal} colorClass="text-blue-500" icon={Beef} />
        <MacroBar label="Carbs" current={carbs} goal={carbsGoal} colorClass="text-orange-500" icon={Wheat} />
        <MacroBar label="Fats" current={fats} goal={fatsGoal} colorClass="text-yellow-500" icon={Droplets} />
      </div>
    </div>
  );
};
